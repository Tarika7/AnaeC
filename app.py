...
def submit():
    ...
    # --- Heart Rate ---
    pulse_count_15s = request.form.get('pulse_count_15s')
    heart_rate_bpm = None
    if pulse_count_15s and pulse_count_15s.isdigit():
        heart_rate_bpm = int(pulse_count_15s) * 4

    # --- BMI Calculation ---
    height_input = request.form.get('height_input')
    ...
    try:
        ...
        if height_unit == "inches":
            height_cm *= 2.54  # Convert inches to cm
    ...
    nail_image = request.files.get('nail_image')

    eye_filename = None
    nail_filename = None
    prediction = "No eye image uploaded"
    advice = {
    ...
    # Save nail image (for future retraining)
    if nail_image and nail_image.filename != '':
        nail_filename = nail_image.filename
        nail_path = os.path.join(app.config['UPLOAD_FOLDER'], nail_filename)
        nail_image.save(nail_path)

    # Predict using eye image
    if eye_image and eye_image.filename != '':
        eye_filename = eye_image.filename
        eye_path = os.path.join(app.config['UPLOAD_FOLDER'], eye_filename)
        eye_image.save(eye_path)

        processed_img = preprocess_image(eye_path)
    ...
    submitted_data = {
        ...
        "Breath Hold Timer (s)": breath_hold_timer,
        "Pulse Count (15s)": pulse_count_15s,
        "Estimated Heart Rate (BPM)": heart_rate_bpm,
        "Height": f"{height_input} {height_unit}",
        "Weight (kg)": weight_kg,
        "BMI": bmi,
        "BMI Category": bmi_category,
        "Eye Image Filename": eye_filename,
        "Nail Image Filename": nail_filename,
